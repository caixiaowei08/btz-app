...
import * as $ from "../../providers/jquery";
...
export class VideoPage {
  ...
  urll:any;
  first:any;
  constructor(private sanitizer: DomSanitizer,public platform:Platform,public navCtrl: NavController,public navParams: NavParams,private httpstorage:HttpStorage,private transfer: Transfer, private file: File) {
    this.playbackRate=1;
    this.seg="s1";
  ...
  isOk(tryOut:any){
    if(tryOut||(this.subject.video&&this.subject.time>=new Date().getTime())) return true;
    else return false;
  }
  getDownClass(id){
